...
let Key = '' // 单引号内自行填写您抓取的Cookie

let DualKey = '' // 如需双账号签到,此处单引号内填写抓取的"账号2"Cookie, 否则请勿填写

let OtherKey = '' // 第三账号或以上的Cookie json串数据, 以下样例为第三第四账号：var OtherKey = '[{"cookie":"pt_key=xxxxxx;pt_pin=yyyyyy"},{"cookie":"pt_key=xxxxxx;pt_pin=yyyyyy"}]'
...
function notify () {
  return new Promise(resolve => {
    try {
      ...
      const Cash = merge.TotalCash && merge.TotalCash.TCash ? `${merge.TotalCash.TCash}红包` : ''
      const Steel = merge.TotalSteel && merge.TotalSteel.TSteel ? `${merge.TotalSteel.TSteel}钢镚` : ''
      const beans = merge.TotalBean && merge.TotalBean.Qbear ? `${merge.TotalBean.Qbear}京豆${Steel ? ', ' : ''}` : ''
      const Money = merge.TotalMoney && merge.TotalMoney.TMoney ? `${merge.TotalMoney.TMoney}现金${Cash ? ', ' : ''}` : ''
      const Subsidy = merge.TotalSubsidy && merge.TotalSubsidy.TSubsidy ? `${merge.TotalSubsidy.TSubsidy}金贴${Money || Cash ? ', ' : ''}` : ''
      const Tbean = bean ? `${bean.toFixed(0)}京豆${steel ? ', ' : ''}` : ''
      const TSteel = steel ? `${steel.toFixed(2)}钢镚` : ''
      const TCash = cash ? `${cash.toFixed(2)}红包${subsidy || money ? ', ' : ''}` : ''
      const TSubsidy = subsidy ? `${subsidy.toFixed(2)}金贴${money ? ', ' : ''}` : ''
      const TMoney = money ? `${money.toFixed(2)}现金` : ''
      const Ts = success ? `成功${success}个${fail || err ? ', ' : ''}` : ''
      const Tf = fail ? `失败${fail}个${err ? ', ' : ''}` : ''
      const Te = err ? `错误${err}个` : ''
      const one = `【签到概览】:  ${Ts + Tf + Te}${Ts || Tf || Te ? '\n' : '获取失败\n'}`
      const two = Tbean || TSteel ? `【签到奖励】:  ${Tbean + TSteel}\n` : ''
      const three = TCash || TSubsidy || TMoney ? `【其他奖励】:  ${TCash + TSubsidy + TMoney}\n` : ''
      const four = `【账号总计】:  ${beans + Steel}${beans || Steel ? '\n' : '获取失败\n'}`
      const five = `【其他总计】:  ${Subsidy + Money + Cash}${Subsidy || Money || Cash ? '\n' : '获取失败\n'}`
      const DName = merge.TotalBean && merge.TotalBean.nickname ? merge.TotalBean.nickname : '获取失败'
      ...
      $nobyda.post(JRSUrl, function (error, response, data) {
        try {
          ...
          if (data.match(/\"resBusiCode\":0/)) {
            ...
            merge.JRSteel.steel = leng ? leng > 9 ? `0.${leng}` : `0.0${leng}` : spare || 0
      ...
      $nobyda.get(GameUrl, async function (error, response, data) {
        try {
          if (error) {
            ...
            const cc = JSON.parse(data)
            if (!id) {
              const status = $nobyda.ItemIsUsed ? '再次检查' : merge.SpeedUp.notify ? '查询本次' : '查询上次'
            ...
            if (cc.message == 'not login') {
              ...
              if (cc.data.task_status == 0 && cc.data.source_id) {
                ...
                const step3 = await JDQueryTaskID(s) // 查询道具ID
                const step4 = await JDUseProps(1000 + s, step3) // 使用道具
...
function GetCookie () {
  try {
    if ($request.method != 'OPTIONS' && $request.headers && $request.url !== 'http://www.apple.com/') {
      const acObj = {}
      // 提取ck数据
      const CV = ($request.headers.Cookie || $request.headers.cookie || '').replace(/ /g, '')
      const ckItems = CV.split(';').filter(s => /^(pt_key|pt_pin)=.+/.test(s)).sort()
      if (ckItems.length == 2) {
      ...
      if (!acObj.cookie) {
        ...
        const [status, seatNo] = chooseSeatNo(acObj.cookie, allCk, /pt_pin=(.+?);/)
        if (status) {
          if (status > 0) {
            let WT = ''
            if (seatNo < 2) {
...
function chooseSeatNo (newCk, allCk, reg) {
  ...
  try {
    const newId = ((newCk || '').match(reg) || ['', ''])[1]
    for (let i = 0, len = allCk.length; i < len; i++) {
      const oldId = ((allCk[i] || '').match(reg) || ['', ''])[1]
      if (oldId) {
